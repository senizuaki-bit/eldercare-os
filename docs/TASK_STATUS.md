...
| M03 Needs/work orders | COMPLETE | feat/m03-needs-workorders | passed | [GitHub PR #2](https://github.com/senizuaki-bit/eldercare-os/pull/2) merged; merge commit `3e5b5fa` |
| M04 Emergency | IN_REVIEW | codex/m04-emergency | passed | [GitHub draft PR #3](https://github.com/senizuaki-bit/eldercare-os/pull/3); implementation commit `86706c7`; P0=0, P1=0 |
...
- Commit/PR: branch `feat/m03-needs-workorders`; milestone commit `d504eb9` (`feat(m03): add auditable needs and work-order workflow`); [GitHub PR #2](https://github.com/senizuaki-bit/eldercare-os/pull/2) was merged into `main` as `3e5b5fa`.
...
- Next milestone prerequisite: satisfied when PR #2 was merged and `codex/m04-emergency` was created from `main`; M04 keeps emergency decisions deterministic and human-controlled rather than treating a health concern or priority work order as a closed emergency.

## M04 acceptance record

- Commit/PR: branch `codex/m04-emergency`; implementation commit `86706c7` (`feat(m04): add auditable emergency response workflow`); [GitHub draft PR #3](https://github.com/senizuaki-bit/eldercare-os/pull/3). The branch contains only the emergency-response milestone and retains M05 navigation as disabled future affordances.
- Migrations and rollback: `20260728000000_emergency` adds emergency source bindings, events, immutable responder-assignment facts, acknowledgements, response milestones, resolutions, supervisor reviews, related-signal links, location snapshots, escalation policies/steps/executions/cancellations, command receipts, family notification preferences/deliveries, and retention/audit foundations. The reviewed `rollback.sql` removes the same M04 objects in dependency-safe order. A clean reset replayed all five migrations, and the final seed succeeded after reset; repeated seeds were stable.
- State machine and deterministic safety: emergency transitions are `OPEN -> ACKNOWLEDGED -> RESPONDING -> RESOLVED -> REVIEWED`; `EN_ROUTE` and `ON_SITE` are ordered immutable milestones within `RESPONDING`. The API validates every transition, version, actor, reason, and required human checklist. An LLM cannot open, escalate, resolve, review, or close an emergency. SLA escalation is driven by versioned facility `EscalationPolicy`/`EscalationStep` records and idempotent jobs.
- APIs and events: administrator list/detail, responder assignment, resolution and supervisor review; elder idempotent signal creation/status; caregiver scoped list/detail, acknowledgement, milestone and resolution; family privacy-filtered list plus versioned notification preferences. Contracts and OpenAPI expose role-specific schemas, pagination, explicit command idempotency and location freshness. M04 event envelopes cover emergency opening, state changes, escalation, family-safe release and cleanup without storing hidden reasoning.
- Command and assignment integrity: responder assignment and family preference replacement use durable command receipts. Replays require the same idempotency key and payload; stale or conflicting commands return `409`. Authorization is rechecked before a receipt replay. Every valid regular or elevated responder assignment creates an immutable fact so reassignment cannot rewrite history.
- Multi-tenancy, authorization and privacy: API transactions recheck organization, facility, permission, active shift, care-team/elder scope and time-bounded emergency elevation. Cross-tenant and out-of-scope records are indistinguishable from missing records. Family reads and deliveries recheck active organization/facility, verified relationship, consent, sharing and preference, and expose only released stage summaries—never precise live location, caregiver trajectory, internal notes, raw device payloads, audio, transcripts or operational checklists.
- Location and retention: every sample carries source, observed time, accuracy and TTL. `CURRENT`, `STALE`, `ROOM_FALLBACK` and `UNKNOWN` are explicit; expired or invalid data is never rendered as live. Future device time is clamped to server receive time, malformed optional location safely degrades while preserving the emergency signal, and bounded retention cleanup deletes expired snapshots idempotently while retaining aggregate-only audit evidence.
- Worker and MQTT: the worker uses BullMQ for SLA, family-notification and retention jobs. The emergency subscriber uses a stable configurable client ID, persistent MQTT session, QoS 1, commit-before-PUBACK, policy-safe retry, poison-message acknowledgement, offline queue replay and duplicate-event idempotency. The IoT simulator supplies readiness/dry-run checks and a deterministic `emergency` publish command; device heartbeat/offline/maintenance behavior remains M05.
- Commands and exact result:
  - `pnpm install --frozen-lockfile` — passed for all 17 workspace projects.
  - `pnpm lint` — passed, 25/25 Turbo tasks plus root checks.
  - `pnpm typecheck` — passed, 25/25 Turbo tasks plus root TypeScript.
  - `pnpm test` — passed, 25/25 tasks; API `211/211`, admin `46/46`, mobile `79/79`, worker `51/51`, contracts `67/67`, DB `35/35`, authz `37/37`, IoT simulator `7/7`, config `7/7`, and all remaining package suites passed.
  - `pnpm test:integration` — passed, 21/21 tasks; API `46/46` including M04 `9/9`, worker `12/12`, and IoT simulator `3/3`.
  - `pnpm test:e2e` — passed `24/24` with two workers at admin 1440/1280 and mobile 375/360 viewports, including the emergency journey, role/privacy denials and serious/critical Axe checks.
  - `pnpm test:e2e:offline` — passed `1/1` against production builds; protected portal data was not restored from cache.
  - `pnpm build` — passed, 17/17 workspace tasks; both Next.js apps produced their production route manifests.
  - `pnpm db:migrate`, repeated migrate/seed, `pnpm db:reset`, and final seed — passed; the final reset replayed M00–M04.
  - `pnpm compose:validate`, `pnpm security:scan`, `pnpm smoke:services`, and `git diff --check` — passed.
  - Independent release/security review — `P0=0`, `P1=0`; no cross-tenant exposure, backend-authorization gap, sensitive log, unaudited emergency change, non-idempotent escalation, destructive migration, M05 scope leak, or unexpected dependency was found.
- Product surfaces and visual QA: admin `/emergencies` and `/emergencies/[id]`; elder `/m/elder/emergency/[id]`; caregiver emergency-first home/list/detail; family emergency summaries and preferences on `/m/family/home`. Same-viewport reference/current evidence is under `docs/design/qa/m04-emergency/`; `design-qa.md` records the 1440×900 and 375×812 comparisons and ends with `final result: passed`.
- Journey mapping: completes the critical secondary journey `emergency signal -> location -> assignment -> escalation -> resolution -> review`. Elder server confirmation, administrator command, caregiver ordered response, family-safe release, MQTT redelivery, stale-location denial, timeout cancellation, idempotency and negative authorization are covered across unit, integration and E2E suites.
- Known limitations: all people, events, devices, locations, messages and phone numbers are fictional. Notifications use a fake provider; a real provider still needs cross-process delivery idempotency and a final authorization check immediately adjacent to external send. The fixed MQTT client ID supports one local emergency subscriber; horizontal scaling needs an explicit ownership strategy. Long-lived retryable head-of-line messages would benefit from a durable inbox/DLQ. Caregiver/admin/family UI retries create fresh command keys except for the elder signal’s response-loss-safe key reuse. The verification machine reports a non-blocking Node engine warning (`24.14.0` versus the declared `>=24.16.0`), and `pg` warns about concurrent `client.query()` calls scheduled for removal in pg 9.
- Next milestone prerequisites: merge [PR #3](https://github.com/senizuaki-bit/eldercare-os/pull/3), create a new M05 branch from accepted `main`, and keep M05 limited to device heartbeat loss, offline alert, maintenance task, fallback patrol, stable recovery and their audit/idempotency tests. Do not reinterpret M04 emergency MQTT/location support as completion of M05.
