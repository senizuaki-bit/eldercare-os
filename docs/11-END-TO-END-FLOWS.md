...
> M03 边界：实现语音请求、Fake AI 草稿、确定性风险规则、人工复核、工单状态流转、护工完成、家属安全摘要和老人评价可靠落库。M03 只使用有效班次/资源范围和当前入住的房间上下文；实时/过期位置判定属于 M06，绩效与报表聚合属于 M09/M16。
...
12. 护工接单；到场追加不可变 `WorkOrderArrival` 事件和时间戳（状态仍为 `ACCEPTED`）；开始处理时才转换为 `IN_PROGRESS`。每次状态转换写入不可变时间线。
