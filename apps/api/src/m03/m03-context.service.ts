...
import type { AuthenticatedRequest, AuthenticatedSession, PortalKind } from '../auth/auth.types.js';
import { resourceNotFound } from '../authorization/tenant-context.service.js';
import { parseSchema } from '../common/parse-schema.js';

export interface M03FacilityContext {
  readonly organizationId: string;
  readonly facilityId: string;
  readonly correlationId: string;
}
@Injectable()
export class M03ContextService {
  ...
  ): Promise<M03FacilityContext> {
    const active = session.principal.activeContext;
    const correlationId = request.correlationId ?? 'unknown-correlation';
    if (active.facilityId === null || !allowedPortals.includes(session.principal.portal)) {
      await this.audit.record({
        organizationId: active.organizationId,
        facilityId: active.facilityId,
        actorUserId: session.userId,
        actorType: 'USER',
        action: 'SECURITY.M03_PORTAL_DENIED',
        outcome: 'DENIED',
        resourceType: 'M03_PORTAL_API',
        reasonCode: active.facilityId === null ? 'FACILITY_CONTEXT_REQUIRED' : 'PORTAL_REQUIRED',
        correlationId,
        metadata: { source: 'm03-portal-boundary' },
      });
